...
CREATE TABLE IF NOT EXISTS public.responsable_area (
    ...
    CONSTRAINT uq_responsable_area_correo UNIQUE (correo),
    -- ck
...
COMMENT ON CONSTRAINT uq_responsable_area_correo ON public.responsable_area IS 'Se asegura de que no haya dos responsables de area con el mismo correo';
-- ck
...
CREATE TABLE IF NOT EXISTS public.ponente (
    ...
    apellido_paterno VARCHAR(50) NOT NULL,
    apellido_materno VARCHAR(50) NOT NULL,
    semblanza VARCHAR(512) NOT NULL,
    -- pk
    ...
    CONSTRAINT ck_ponente_semblanza_no_vacio CHECK (btrim(semblanza) <> ''),
    -- uq
    CONSTRAINT uq_ponente_nombre_completo UNIQUE (nombre, apellido_paterno, apellido_materno),
    CONSTRAINT uq_ponente_semblanza UNIQUE (semblanza)
);
-- documentacion
-- tabla
...
COMMENT ON COLUMN public.ponente.semblanza IS 'Ruta del archivo pdf donde se almacena la semblanza del ponente';
-- ck
...
COMMENT ON CONSTRAINT ck_ponente_semblanza_no_vacio ON public.ponente IS 'Se asegura de que el valor de la semblanza no esté vacío';
-- uq
...
COMMENT ON CONSTRAINT uq_ponente_semblanza ON public.ponente IS 'Se asegura de que no haya dos ponentes con la misma semblanza';
...
CREATE TABLE IF NOT EXISTS public.area (
    ...
    activo BOOLEAN DEFAULT True NOT NULL,
    id_responsable_area SMALLINT NOT NULL,
    -- pk
...
CREATE TABLE IF NOT EXISTS public.evento (
    ...
    presencial BOOLEAN DEFAULT True NOT NULL,
    online BOOLEAN DEFAULT False NOT NULL,
    estatus VARCHAR(50) DEFAULT 'pendiente' NOT NULL,
    ...
    CONSTRAINT ck_evento_estatus CHECK (estatus IN ('pendiente', 'autorizada', 'realizada', 'cancelada')),
...
COMMENT ON COLUMN public.evento.estatus IS 'Estatus del evento (pendiente, autorizada, realizada, cancelada)';
...
COMMENT ON CONSTRAINT ck_evento_estatus ON public.evento IS 'Valida que solo se registren los estatus pendiente, autorizada, realizada y cancelada';
...
CREATE TABLE IF NOT EXISTS public.grado (
    id_grado SMALLINT GENERATED ALWAYS AS IDENTITY,
    ...
    -- uq
    CONSTRAINT uq_grado_titulo_academico UNIQUE (titulo,id_nivel,id_institucion)
);
-- documentacion
-- tabla
...
CREATE TABLE IF NOT EXISTS public.equipamiento (
    ...
    foto VARCHAR(512) DEFAULT '/equipamiento/fotos/equipamiento_default.png' NOT NULL,
    nombre VARCHAR(100) NOT NULL,
    ...
    CONSTRAINT fk_area FOREIGN KEY (id_area) REFERENCES public.area (id_area) ON DELETE RESTRICT ON UPDATE CASCADE,
    -- ck
    CONSTRAINT ck_equipamiento_activo_id_area CHECK (NOT activo OR id_area IS NOT NULL),    
    CONSTRAINT ck_equipamiento_cantidad CHECK (cantidad > 0),
...
COMMENT ON COLUMN public.equipamiento.id_area IS 'Area que esta a cargo del equipamiento';
-- ck
COMMENT ON CONSTRAINT ck_equipamiento_activo_id_area ON public.equipamiento IS 'Se asegura de que si el equipamiento esta activo, entonces debe tener un area asignada';
COMMENT ON CONSTRAINT ck_equipamiento_cantidad ON public.equipamiento IS 'Se asegura de que la cantidad sea un valor positivo mayor a cero';
...
-- ponentexgrado
---------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.ponentexgrado (
    id_ponente INTEGER,
    id_grado SMALLINT,
    -- pk
    CONSTRAINT pk_ponente_x_grado PRIMARY KEY (id_ponente, id_grado),
    -- fk
    CONSTRAINT fk_ponente FOREIGN KEY (id_ponente) REFERENCES public.ponente (id_ponente) ON DELETE RESTRICT ON UPDATE CASCADE,
...
-- documentacion
-- tabla
COMMENT ON TABLE public.ponentexgrado IS 'Registra los grados que tienen cada uno de los ponentes';
-- atributos
COMMENT ON COLUMN public.ponentexgrado.id_ponente IS 'Identificador del ponente que participa en al menos un evento';
COMMENT ON COLUMN public.ponentexgrado.id_grado IS 'Identificador del grado academico del ponente';
...
CREATE TABLE IF NOT EXISTS public.fotografia (
    ...
    -- uq
    CONSTRAINT uq_fotografia_fotografia UNIQUE (id_recinto,fotografia)
);
-- documentacion
-- tabla
...
-- uq
COMMENT ON CONSTRAINT uq_fotografia_fotografia ON public.fotografia IS 'Se asegura de que un recinto no tenga las mismas fotografias';
